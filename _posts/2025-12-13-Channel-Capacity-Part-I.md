...
Consider the capacity expression that maximizes the mutual information between input and output of a channel for all input distribution $f(x)$ satisfying positive definite covariance matrix $Q \ge 0$, and $Tr(Q)=P$ as,
...
Let $\widetilde{Q}=V'QV$ and the unitary matrix V multiplication does not change the statistics of $\widetilde{Q}$ matrix because $Tr(\widetilde{Q})=Tr(V'QV)=Tr(Q)$. 
